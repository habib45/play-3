error id: file://<WORKSPACE>/app/repositories/CategoryRepository.java
file://<WORKSPACE>/app/repositories/CategoryRepository.java
...
```java
offset: 1
uri: file://<WORKSPACE>/app/repositories/CategoryRepository.java
text:
```scala
p@@ackage repositories
...
class ProductRepository @Inject()() {
  ...
  def delete(id: Long): Boolean = {
    val product = Product.find.byId(id)
    if (product != null) {
      product.delete()
      true
    } else {
...
QDox parse error in file://<WORKSPACE>/app/repositories/CategoryRepository.java